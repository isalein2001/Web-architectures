...
export default function WorkoutLogger() {
  ...
  const startFreestyle = () => {
    setActivePlan({ id: null, name: t('Freestyle Workout') });
    setLogs([]);
  };
    ...
    return (
      <div className="workout-logger">
        <header className="page-header">
          <div>
            <h1 className="page-title">{t('Start a Workout')}</h1>
            <p style={{color: 'var(--text-secondary)'}}>{t('Choose a routine or go freestyle')}</p>
          </div>
        </header>

        <div className="grid grid-cols-3">
          <div className="card" style={{cursor: 'pointer', textAlign: 'center', borderColor: 'var(--accent-primary)'}} onClick={startFreestyle}>
            <h3 style={{fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '0.5rem'}}>{t('Freestyle Workout')}</h3>
            <p style={{color: 'var(--text-secondary)'}}>{t('Log any exercise on the fly.')}</p>
          </div>
          {plans.map(plan => (
            <div key={plan.id} className="card" style={{cursor: 'pointer'}} onClick={() => startPlan(plan)}>
              ...
              <p style={{color: 'var(--text-secondary)', fontSize: '0.875rem'}}>{plan.exercises?.length || 0} {t('exercises')}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="active-workout">
      <header className="page-header">
        <div>
          ...
          <p style={{color: 'var(--text-secondary)'}}>{t('Session in progress')}</p>
        </div>
        <button className="btn btn-primary" onClick={finishWorkout}>
          <Save size={18} /> {t('Finish Routine')}
        </button>
      </header>
          ...
          <div key={exName} className="card" style={{marginBottom: '1.5rem'}}>
            ...
            <div style={{display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
               <div style={{display: 'grid', gridTemplateColumns: '50px 1fr 1fr 60px', gap: '1rem', paddingBottom: '0.5rem', borderBottom: '1px solid var(--border-color)', color: 'var(--text-secondary)', fontSize: '0.875rem', fontWeight: '600'}}>
                 <div>{t('Set')}</div>
                 <div>{t('Weight (kg/lb)')}</div>
                 <div>{t('Reps')}</div>
                 <div style={{textAlign: 'center'}}>{t('Done')}</div>
               </div>
               ...
               {exLogs.map(log => (
                 <div key={log.id} style={{display: 'grid', gridTemplateColumns: '50px 1fr 1fr 60px', gap: '1rem', alignItems: 'center', opacity: log.completed ? 0.6 : 1, transition: 'var(--transition)'}}>
                   ...
                   <input className="input" type="number" placeholder={t('Weight')} value={log.weight} onChange={e => updateLog(log.id, 'weight', e.target.value)} disabled={log.completed} />
                   <input className="input" type="number" placeholder={t('Reps')} value={log.reps} onChange={e => updateLog(log.id, 'reps', e.target.value)} disabled={log.completed} />
                   <button 
      ...
      <div className="card" style={{marginBottom: '2rem'}}>
        <h3 style={{fontSize: '1.25rem', marginBottom: '1rem'}}>{t('Add Freestyle Exercise')}</h3>
        <div style={{display: 'flex', gap: '1rem'}}>
          <input className="input" style={{flex: 1}} placeholder={t('Exercise Name...')} value={currentExercise} onChange={e => setCurrentExercise(e.target.value)} />
      ...
      <div className="card" style={{marginBottom: '2rem'}}>
        <h3 style={{fontSize: '1.25rem', marginBottom: '1rem'}}>{t('Session Notes')}</h3>
        <textarea className="input" rows="3" placeholder={t('How did this workout feel?')} value={notes} onChange={e => setNotes(e.target.value)}></textarea>
      </div>
